...
contract("MyWallet", function(accounts) {
    it('should be possible to put money inside', function() {
        ...
        return wallet.deployed().then(function(instance) {
            ...
            return contractInstance.sendTransaction({value: web3.toWei(0.001, 'ether'), address:contractInstance.address, from: accounts[0]});
        }).then(function(result) {
            assert.equal(web3.eth.getBalance(contractInstance.address).toNumber(), web3.toWei(0.001, 'ether'), 'The Balance is the same');
        });
    });
    ...
    //     return wallet.deployed().then(function(instance) {
    //         walletInstance = instance;
    //         return walletInstance.spendMoneyOn(accounts[1], web3.toWei(0.001,'ether'), "Because I need money", {from: accounts[1]});
    //     }).then(function() {
    //         assert.equal(web3.eth.getBalance(walletInstance.address).toNumber(), web3.toWei(0.001, 'ether'), 'Balance is now 5 ether less than before');
    //     });
    // });
});